...
import { Send, Search, Plus, Bot, MessageSquare, Sparkles, CheckCheck } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { supabase } from '../lib/supabase';
import type { Message, User, BotMessage } from '../types/database';
import { useScrollAnimation } from '../hooks/useAnimations';

type ConversationUser = Pick<User, 'id' | 'full_name' | 'avatar_url' | 'email' | 'updated_at'>;

// Shape shared by real (human-to-human) messages and synthesized bot
// messages, so the thread UI can render both without caring which table
// they came from.
...
interface Conversation {
  ...
  lastMessage: ThreadItem | null;
  unread: number;
  isBot?: boolean;
}

function botMessageToThreadItem(m: BotMessage, botId: string, userId: string): ThreadItem {
  return {
    id: m.id,
    sender_id: m.role === 'user' ? userId : botId,
    content: m.content,
...
export function MessagesPage() {
  ...
  async function loadConversations() {
    ...
    const humanMessages: Message[] = data || [];

    // Bot ids are frontend-only constants (not rows in `users`), so they
    // must never be mixed into a query against the `users` table — that
    // column is `uuid`, and filtering it with a non-uuid string like
    // 'bot-assistant' makes the whole request fail.
    const userIds = new Set<string>();
    humanMessages.forEach((m: Message) => {
      const other = m.sender_id === user.id ? m.receiver_id : m.sender_id;
      userIds.add(other);
    });

    const { data: latestBotMessages } = await supabase.from('bot_messages')
      .select('*')
      .eq('user_id', user.id)
      .order('created_at', { ascending: false });

    const botLastByBotId = new Map<string, BotMessage>();
    (latestBotMessages || []).forEach((m: BotMessage) => {
      if (!botLastByBotId.has(m.bot_id)) botLastByBotId.set(m.bot_id, m);
    });

    const botConvs: Conversation[] = BOT_USERS.map(bot => {
      const last = botLastByBotId.get(bot.id);
      return {
        user: bot,
        lastMessage: last ? botMessageToThreadItem(last, bot.id, user.id) : null,
        unread: 0,
        isBot: true,
      };
    });
    ...
    const humanConvs: Conversation[] = realUsers.map((u: any) => {
      const msgs = humanMessages.filter((m: Message) => m.sender_id === u.id || m.receiver_id === u.id);
    ...
    setConversations([...humanConvs, ...botConvs]);
  }
  ...
  async function selectConversation(u: ConversationUser) {
    ...
    if (isBot) {
      const { data } = await supabase.from('bot_messages')
        ...
        .eq('bot_id', u.id)
        .order('created_at');
      setThreadMessages((data || []).map((m: BotMessage) => botMessageToThreadItem(m, u.id, user.id)));
  ...
  const sendMessage = useCallback(async () => {
    ...
    if (selected.id.startsWith('bot-')) {
      const userMsg: ThreadItem = { id: crypto.randomUUID(), sender_id: user.id, content, created_at: new Date().toISOString() };
      setThreadMessages(prev => [...prev, userMsg]);
      await supabase.from('bot_messages').insert({ user_id: user.id, bot_id: selected.id, role: 'user', content } as never);
      loadConversations();
  ...
  const visibleConversations = conversationFilter.trim()
    ? conversations.filter(({ user: u }) =>
        u.full_name.toLowerCase().includes(conversationFilter.toLowerCase()) ||
        u.email.toLowerCase().includes(conversationFilter.toLowerCase())
      )
    : conversations;
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 ? (
            ...
            <div className="p-6 text-center">
              ...
              <p className="text-text-muted text-sm font-medium">Nenhuma conversa encontrada</p>
              <p className="text-text-muted/60 text-xs mt-1">Tente outro termo de busca</p>
            </div>
          ) : (
            visibleConversations.map(({ user: u, lastMessage, unread, isBot }) => (
              <button
                key={u.id}
