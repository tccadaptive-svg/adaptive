...
import { Search, Star, Phone, Globe, X, CheckCircle, MapPin, RefreshCw } from 'lucide-react';
...
// Automatically fits the map viewport so every filtered gym marker (plus the
// user's location, when known) stays visible whenever the result set changes.
function FitBoundsController({ gyms, userCenter }: { gyms: Gym[]; userCenter: [number, number] | null }) {
  const map = useMap();

  useEffect(() => {
    const points: [number, number][] = gyms.map(g => [g.latitude, g.longitude]);
    ...
    <div className="h-screen flex flex-col lg:flex-row">
      ...
      <div className="flex-1 relative gradient-border">
        ...
        <div className="absolute top-4 left-4 right-4 z-[400] space-y-2">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border border-white/10 bg-bg-secondary/80 backdrop-blur-sm text-text-muted w-fit">
            <MapPin size={12} className="text-accent-blue" aria-hidden="true" />
            <span aria-live="polite">
              {filtered.length} {filtered.length === 1 ? 'academia encontrada' : 'academias encontradas'}
            </span>
          </div>
        </div>
        ...
        <div ref={mapRef} className="w-full h-full">
          ...
          >
            ...
            <FitBoundsController gyms={filtered} userCenter={userCenter} />
            {userCenter && (
                ...
                <Popup>
                  <div className="min-w-[160px]">
                    ...
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <span className="text-xs text-text-muted">{gym.rating.toFixed(1)}</span>
                      {gym.verified && <CheckCircle size={12} className="text-success ml-1" />}
                    </div>
                  </div>
                </Popup>
              </Marker>
